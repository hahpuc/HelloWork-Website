...
export default class CompanyInfoList extends AppComponentBase<ICompanyInfoProps, ICompanyInfoState> {
    ...
    getCompanyInfoById = (comID: number) => {
        ...
        this.props.CompanyInfoStore.getCompanyInfoByID(dto);
    }
    async getRecruiterInfoByID() {
        ...
        this.idRecuiter =  Number(pathLast[pathLast.length - 1]);
        let dto:EntityDto={ id:this.idRecuiter};
    ...
    public render() {
        ...
        const recruitmentPost = recruitmentPosts.items.map((value : IRecruitmentPostItem, index : number) => 
        {
            ...
            return (   
                <div style={{marginLeft:16,marginRight:32,marginTop:16, alignItems:"center",justifyContent:"center"}}  className="row-margin-frames">
                        ...
                        <h2  style={{marginLeft :52,fontWeight:900, fontSize:25}} ><b>Danh sách tin tuyển dụng</b></h2>
                        {
                             (this.state.Actor==="Chủ tài khoản") ?
                                <div style={{marginRight: 16}}>
                                    <Button className="btnUpdate" type={"primary"}  onClick={()=>{this.setState({redirect : "/recruimentPosts"})}} >Quản lý tin</Button>
                                    <Button className="btnUpdate" type={"primary"} onClick={()=>{this.setState({redirect : "/tin-tuyen-dung/dang-tin/"+this.state.selectedID})}} >Tạo tin mới</Button>
                                </div>: null
                        }
                    </div>
