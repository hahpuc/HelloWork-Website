...
import {Button, Avatar, Rate,Input , Modal, message,Form, Col, Row, Upload } from "antd";
...
import ButtonsEditAndDelete from "../../Component/ButtonsEditAndDelete/ButtonsEditAndDelete";
import ModeButtons from "../../Component/ModeButtons/ModeButtons";
...
export default class JobSeekerList extends AppComponentBase<IJobSeekerListProps, IJobSeekerListState> {
    ...
    checkUpdateJobSeeker = () => {
       ...
       if (UpdatejobSeeker.name === "")
        {
            message.error("Bạn phải nhập cho trường tên");
            return;
        }

        if (UpdatejobSeeker.birthday === "")
        {
            message.error("Bạn phải nhập cho trường ngày sinh");
            return;
        }

        if (UpdatejobSeeker.sex === "")
        {
            message.error("Bạn phải nhập cho trường giới tính");
            return;
        }

        if (UpdatejobSeeker.workLocation === "")
        {
            message.error("Bạn phải nhập cho trường nơi làm việc");
            return;
        }

        if (UpdatejobSeeker.address === "")
        {
            message.error("Bạn phải nhập cho trường địa chỉ");
            return;
        }

        if (UpdatejobSeeker.email === "")
        {
            message.error("Bạn phải nhập cho trường email");
            return;
        }

        if (UpdatejobSeeker.phoneNumber === "")
        {
            message.error("Bạn phải nhập cho trường số điện thoại");
            return;
        }

        if (UpdatejobSeeker.expertise === "")
        {
            message.error("Bạn phải nhập cho trường chuyên môn");
            return;
        }

        if (UpdatejobSeeker.field === "")
        {
            message.error("Bạn phải nhập cho trường lĩnh vực hoạt động");
            return;
        }

        if (UpdatejobSeeker.facebook === "")
        {
            message.error("Bạn phải nhập cho trường facebook");
            return;
        }
    }
    //#endregion
...
//#endregion
    ...
    public render() {
        ...
        if (this.state.redirect !== "")
            return <Redirect push to="/job-management"/>;
        //#endregion
                ...
                <div style={{margin :30}}  className="row-margin-frames">
                    <div className="study-area" style={{borderStyle:"none",backgroundColor:"whitesmoke"}}>
                        ...
                        <ButtonsEditAndDelete 
                            ...
                            onClickEdit={() => this.editEducation(value)}
                            onClickDelete={ () => { this.setState({sectionSelectedDelete:"Học vấn", idItemSelectedDelete: value.id}); this.showDeleteConfirm(this)}}>
                        </ButtonsEditAndDelete>
                    </div>
                 </div>
            );
            ...
            return(
                <div className="section">
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                        ...
                        <ModeButtons 
                            isEditing={this.state.isEditingEducation}
                            onClickAdd={this.addEducation}
                            onClickEdit={() => {this.setState({isEditingEducation: true})}}
                            onClickDone={() => {this.setState({isEditingEducation: false})}}>
                        </ModeButtons>
                    </div>
            ...
            return (                
                <div style={{margin :30}}  className="row-margin-frames">
                    <div className="study-area" style={{borderStyle:"none",backgroundColor:"whitesmoke"}}>
                        ...
                        <ButtonsEditAndDelete 
                            isEditing={this.state.isEditingWorkExperience} 
                            onClickEdit={() => this.editWorkExperience(value)}
                            onClickDelete={ () => { this.setState({sectionSelectedDelete:"Kinh nghiệm", idItemSelectedDelete: value.id}); this.showDeleteConfirm(this)}}>
                        </ButtonsEditAndDelete>
                    </div>
                </div>
            );
            ...
            return(
                <div className="section">
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                        ...
                        <ModeButtons 
                            isEditing={this.state.isEditingWorkExperience}
                            onClickAdd={this.addWorkExperience}
                            onClickEdit={() => {this.setState({isEditingWorkExperience: true})}}
                            onClickDone={() => {this.setState({isEditingWorkExperience: false})}}>
                        </ModeButtons>
                    </div>
                ...
                <div style={{margin :30}}  className="row-margin-frames">
                    <div className="study-area" style={{borderStyle:"none",backgroundColor:"whitesmoke"}}>
                        ...
                        <ButtonsEditAndDelete 
                            ...
                            onClickEdit={() => this.editSkill(value)}
                            onClickDelete={ () => { this.setState({sectionSelectedDelete:"Kỹ năng", idItemSelectedDelete: value.id}); this.showDeleteConfirm(this)}}>
                        </ButtonsEditAndDelete>
                    </div>
                </div>
            );
            ...
            return(
                <div className="section">
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                        ...
                        <ModeButtons 
                            isEditing={this.state.isEditingSkill}
                            onClickAdd={this.addSkill}
                            onClickEdit={() => {this.setState({isEditingSkill: true})}}
                            onClickDone={() => {this.setState({isEditingSkill: false})}}>
                        </ModeButtons>
                    </div>
            ...
            return (                
                <div style={{margin :30}}  className="row-margin-frames">
                    <div className="study-area" style={{borderStyle:"none",backgroundColor:"whitesmoke"}}>
                        ...
                        <ButtonsEditAndDelete 
                            isEditing={this.state.isEditingOrientation} 
                            onClickEdit={() => this.editOrientation(value)}
                            onClickDelete={ () => { this.setState({sectionSelectedDelete:"Định hướng", idItemSelectedDelete: value.id}); this.showDeleteConfirm(this)}}>
                        </ButtonsEditAndDelete>
                    </div>
                 </div>
            );
            ...
            return(
                ...
                <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                        ...
                        <ModeButtons 
                            isEditing={this.state.isEditingOrientation}
                            onClickAdd={this.addOrientation}
                            onClickEdit={() => {this.setState({isEditingOrientation: true})}}
                            onClickDone={() => {this.setState({isEditingOrientation: false})}}>
                        </ModeButtons>
                    </div>
            ...
            return (                
                <div style={{margin :30}}  className="row-margin-frames">
                    <div className="study-area" style={{borderStyle:"none",backgroundColor:"whitesmoke"}}>
                        ...
                        <ButtonsEditAndDelete 
                            isEditing={this.state.isEditingAchievement} 
                            onClickEdit={() => this.editAchievement(value)}
                            onClickDelete={ () => { this.setState({sectionSelectedDelete:"Thành tựu", idItemSelectedDelete: value.id}); this.showDeleteConfirm(this)}}>
                        </ButtonsEditAndDelete>
                    </div>
                </div>
            );
            ...
            return(
                <div className="section">
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
                        ...
                        <ModeButtons 
                            isEditing={this.state.isEditingAchievement}
                            onClickAdd={this.addAchievement}
                            onClickEdit={() => {this.setState({isEditingAchievement: true})}}
                            onClickDone={() => {this.setState({isEditingAchievement: false})}}>
                        </ModeButtons>
                    </div>
        ...
        return (
            <div style={{ margin: "auto", marginTop: "20px", width: "60%", minWidth:900 }}>
                ...
                <div >
                    ...
                    <div className="whiteHeader">
                        ...
                        <div className="avatarJobSeeker">
                            ...
                            <Upload showUploadList={false}>
                                <Button className="btnEditAvatar" type="default" shape="circle" icon="edit" onClick={this.editAvatar}></Button>
                            </Upload>
                        </div>
                        <div>
                            <div className="banner" style={{marginLeft:220,marginTop:-80}}>
                            ...
                            <div style = {{justifyContent:"flex-end", display:"flex"}}>
                                <button className="buttonEditHeader" onClick={()=>{this.setState({redirect : "/job-management"})}} style={{marginRight: 50, height:40, width:200, backgroundColor:"#39bd5a", color:"white"}}>Quản lý việc làm</button>
                            </div>
                        </div>
                ...
                <CustomModal className="Custom_Modal_Out_Layout" style={{margin :0}} 
                    shadow={true}
                    type="custom"
                ...
                >
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Họ và tên')}{...formItemLayout} >
                        <Input  style = {{fontSize : 13}} defaultValue={this.state.UpdatejobSeeker.name} 
                        onChange={(e) => {
                            let temp_name : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, name : temp_name}});
                            }}/>
                    </FormItem>
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Ngày sinh')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.birthday.substr(8,2) + "/" + jobSeeker.birthday.substr(5,2) + "/" + jobSeeker.birthday.substr(0,4)} 
                        onChange={(e) => {
                    ...
                    <FormItem className="Fontstyle_formitem"  style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Giới tính')} {...formItemLayout}>
                    ...
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Nơi làm việc')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.workLocation}
                        onChange={(e) => {
                    ...
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}   label={('Dòng giới thiệu')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.description} 
                        onChange={(e) => {
                    ...
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Địa chỉ')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.address} 
                        onChange={(e) => {
                            let temp_address : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, address : temp_address}});
                            }}/>
                    </FormItem>
                    <FormItem className="Fontstyle_formitem"  style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Email liên hệ')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.email}
                        onChange={(e) => {
                            let temp_email : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, email : temp_email}});
                            }}/>
                    </FormItem>
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}   label={('Số điện thoại')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.phoneNumber} 
                        onChange={(e) => {
                    ...
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Chuyên môn')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.expertise}  
                        onChange={(e) => {
                    ...
                    <FormItem  className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Lĩnh vực hoạt động')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.field}   
                        onChange={(e) => {
                            let temp_field : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, field : temp_field}});
                            }} />
                    </FormItem>
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Facebook')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.facebook} 
                        onChange={(e) => {
                            let temp_facebook : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, facebook : temp_facebook}});
                            }}/>
                    </FormItem>
                    <FormItem className="Fontstyle_formitem"  style={{marginRight :20, marginTop:5, marginBottom: 0}}  label={('Github')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.github}  
                        onChange={(e) => {
                            let temp_github : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, github : temp_github}});
                            }}/>
                    </FormItem>
                    <FormItem className="Fontstyle_formitem" style={{marginRight :20, marginTop:5, marginBottom: 0}} label={('Twitter')} {...formItemLayout}>
                        <Input style = {{fontSize : 13}} defaultValue={jobSeeker.twitter} 
                        onChange={(e) => {
                            let temp_twitter : string  = e.target.value;
                            this.setState({UpdatejobSeeker : {...this.state.UpdatejobSeeker, twitter : temp_twitter}});
                            }}/>
                    </FormItem>
                    <div style={{marginRight :15, marginTop:5, marginBottom: 0}}  className="Custom_Modal_Footer">
                        <div style={{ display: "flex"}  } >
                                <button style={{margin :10}}  className="Simple_White_Button"  onClick={() => {  this.setState({ isEditJobSeekerPopupOpen: false }) }} >Hủy thay đổi</button>
                                <button style={{marginTop :10}}  className="Simple_Blue_Button" 
                                    onClick={() => {
                ...
                {/* Modal thêm định hướng công việc */}
                <ModalEducation className="Custom_Modal_Out_Layout3" style={{margin :10}} 
                        shadow={true}
                        type="custom"
    ...
    checkUpdateEducation = () => {
        const  {selectedEducation} = this.state;
        if (selectedEducation.majors === "")
        {
            message.error("Bạn phải nhập cho trường ngành");
            return;
        }
        if (selectedEducation.school === "")
        {
            message.error("Bạn phải nhập cho trường tên trường");
            return;
        }
    }

    async UpdateEducation() {
        this.checkUpdateEducation();
    ...
    checkUpdateExperience = () => {
        ...
        {
            message.error("Bạn phải nhập cho trường tên công ty")
            return;
        }
        if (selectedExperience.role === "")
        {
            message.error("Bạn phải nhập cho trường vị trí")
            return;
        }
        // if(Number.isNaN(selectedExperience.startYear))
        // {
        //     this.setState({Error: "Bạn phải nhập cho trường năm bắt đầu"});
        //     this.showError(this);
        //     return;
        // }
        // if (selectedExperience.endYear === null)
        // {
        //     this.setState({Error: "Bạn phải nhập cho trường năm kết thúc"});
        //     this.showError(this);
        //     return;
        // }
    }
    ...
    checkUpdateSkill = () => {
        ...
        {
            message.error("Bạn phải nhập cho trường kỹ năng");
            return;
        }
    }

    async UpdateSkill() {
        this.checkUpdateSkill();
    ...
    checkUpdateOrientation = () => {
        ...
        {
            message.error("Bạn phải nhập cho trường định hướng công việc");
            return;
        }
    }

    async UpdateOrientation() {
        this.checkUpdateOrientation();
    ...
    checkUpdateAchievement = () => {
        ...
        if (selectedAchievement.achievementName === "")
        {
            message.error("Bạn phải nhập cho trường tên thành tựu, chứng nhận");
            return;
        }
        if (selectedAchievement.organization === "")
        {
            message.error("Bạn phải nhập cho trường tổ chức");
            return;
        }
    }
