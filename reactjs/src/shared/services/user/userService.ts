...
class UserService {
  ...
  public async register(createUserInput: any) {
    let result = await http.post('/api/userAccount/CreateEmployee', createUserInput);
    return result.data.result;
  }

  public async registerEmployer(input: any) {
    const result = await http.post('/api/userAccount/CreateEmployer', input);
    return result.data.result;
  }
  ...
  public async forgetPassword(input: any) {
    let result = await http.post('/api/userAccount/ForgetPassword', input);
    return result.data.result;
  }
}
